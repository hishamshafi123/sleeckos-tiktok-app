...
export default function ClipMixerPage() {
  ...
  const [selectedCampaignId, setSelectedCampaignId] = useState<string>(queryCampaignId);
  ...
  const [selectedFolderId, setSelectedFolderId] = useState<string>(queryFolderId);
  const [selectedFolderIds, setSelectedFolderIds] = useState<string[]>(queryFolderId ? [queryFolderId] : []);
  ...
  useEffect(() => {
    if (queryFolderId) {
      ...
      setSelectedFolderIds(prev => prev.includes(queryFolderId) ? prev : [...prev, queryFolderId]);
    }
  }, [queryFolderId]);
  ...
  // Fetch folders for selected section and campaign
  const fetchFolders = async (sectionId: string) => {
    if (!sectionId) return;
    setIsLoadingFolders(true);
    try {
      const campaignParam = selectedCampaignId ? selectedCampaignId : "null";
      const res = await fetch(`/api/managed/clip-mixer/folders?sectionId=${sectionId}&campaignId=${campaignParam}`);
  ...
  // Section and Campaign dependency triggers
  useEffect(() => {
    if (selectedSectionId) {
      fetchFolders(selectedSectionId);
    }
  }, [selectedSectionId, selectedCampaignId]);
    ...
    const startBatchRender = async () => {
      const res = await fetch("/api/managed/clip-mixer/batches", {
        ...
        body: JSON.stringify({
          ...
          variationStrength: Number(variationStrength),
          campaignId: selectedCampaignId || null,
        }),
      });
      const data = await res.json();
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 border-b border-white/5 pb-6">
        ...
        <div className="flex flex-wrap items-center gap-4">
          ...
          <div className="flex items-center gap-3 bg-[#0f0f18] p-2 rounded-2xl border border-white/5 shadow-inner">
            <span className="text-xs text-gray-500 font-bold uppercase tracking-wider pl-2">Campaign:</span>
            <select
              value={selectedCampaignId}
              onChange={(e) => setSelectedCampaignId(e.target.value)}
              className="bg-[#07070c] border border-white/5 rounded-xl px-4 py-2 text-white font-semibold text-sm focus:outline-none focus:border-purple-500 transition-all cursor-pointer min-w-[200px]"
            >
        ...
        <div className="max-w-4xl mx-auto space-y-6">
          ...
          <div className="bg-[#0c0c14] border border-white/5 rounded-3xl p-6 shadow-2xl space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <label className="text-xs text-gray-400 font-bold uppercase tracking-wider">Variation Strength:</label>
                  <span className="text-sm font-bold text-purple-400">Level {variationStrength} / 5</span>
                </div>
                <input
                  type="range"
                  min="1"
                ...
                />
                <p className="text-[10px] text-gray-500 leading-normal">
                  {variationStrength === 1 && "Level 1: Minimal slice trim randomness. Sequential rotation, no shuffle."}
                  {variationStrength === 2 && "Level 2: Low variation. Trims picked from start, middle, or end. Minimal slice shuffling."}
                  {variationStrength === 3 && "Level 3: Standard variation. Random shuffling, standard segment trims +/- 2.0s."}
                  {variationStrength === 4 && "Level 4: High variation. Completely shuffled slices, wider trims, variable slice durations (2.5s-5.5s)."}
                  {variationStrength === 5 && "Level 5: Extreme variation. Variable slice counts and durations (2.0s-6.0s), fully randomized trims."}
                </p>
              </div>
